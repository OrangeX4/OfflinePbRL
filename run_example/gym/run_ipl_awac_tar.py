...
from offlinepbrl.buffer import TrajectoryBuffer, PrefBuffer
from offlinepbrl.utils.logger import Logger, make_log_dirs
from offlinepbrl.policy_trainer import MFPolicyTrainer
from offlinepbrl.policy import IPLAWACTARPolicy

"""
IPL-AWAC with Trajectory Adherence Regularization (TAR)

TAR adds a trajectory-level regularization that penalizes deviations from
dataset trajectories: L_TAR = (sum_t ||π(s_t) - a_t||^2)^2

Suggested hypers:
temperature=0.3
reward_reg=0.5
tar_coef=1.0 (adjust based on task)
"""
...
def get_args():
    ...
    parser.add_argument("--algo_name", type=str, default="ipl_awac_tar")
    ...
    parser.add_argument("--tar_coef", type=float, default=1.0, help="TAR regularization coefficient")
    parser.add_argument("--tar_clip_max", type=float, default=10.0, help="Max per-step deviation")
    parser.add_argument("--traj_batch_size", type=int, default=8, help="Number of trajectories for TAR")
...
def train(args=get_args()):
    ...
    for m in list(actor.modules()) + list(critic_q1.modules()) + list(critic_q2.modules()):
    ...
    # create IPL-AWAC-TAR policy
    policy = IPLAWACTARPolicy(
        actor,
        critic_q1,
        critic_q2,
        actor_optim,
        ...
        tar_coef=args.tar_coef,
        tar_clip_max=args.tar_clip_max,
    )

    # create trajectory buffer (for TAR)
